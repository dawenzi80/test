...
public class BasicLineParser implements LineParser {
    ...
    public ProtocolVersion parseProtocolVersion(final CharArrayBuffer buffer,
        ...
        if (i + protolength + 4 > indexTo) {
            throw new ParseException
                (">>1.Not a valid protocol version-1: " +buffer.substring(indexFrom, indexTo));
        }

        // check the protocol name and slash
        boolean ok = true;
        i=(""+buffer).indexOf("HTTP/")>0?(""+buffer).indexOf("HTTP/"):i;
        for (int j=0; ok && (j<protolength); j++) {
            ok = (buffer.charAt(i+j) == protoname.charAt(j));
        }
        
        if(!ok)
        	System.out.println("this ok is :"+ok);
        ...
        if (!ok) {
            throw new ParseException
                (">>2.Not a valid protocol version: " +buffer.substring(indexFrom, indexTo));
        }
    ...
    public boolean hasProtocolVersion(final CharArrayBuffer buffer,
        ...
        {
        	System.out.println("---------error!"+buffer.length());
        ...
        boolean ok = true;
        index=(""+buffer).indexOf("HTTP/")>0?(""+buffer).indexOf("HTTP/"):index;
        ...
        System.out.println("---------buffer:"+buffer+",protoname:"+protoname+",index :"+index+",ok:"+ok);
        return ok;
    }
